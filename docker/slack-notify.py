#!/usr/bin/env python3
"""
Slack notification — the agent's "action" step. Coral is read-only and CANNOT
post to Slack, so the agent calls this script (chat.postMessage via bot token).
Usage: python3 slack-notify.py "message text"
Env: SLACK_BOT_USER_OAUTH_TOKEN (or SLACK_BOT_TOKEN), SLACK_CHANNEL (default incidents-alerts)
"""
import sys
import os
import requests
